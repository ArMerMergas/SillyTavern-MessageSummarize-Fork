...
{{message}}
`
const default_aggressive_prompt = `Summarize this in one brief factual sentence. Names and key events only. No details.
...
const default_settings = {
    ...
    aggressive_depth: 200,  // messages deeper than this use aggressive prompt
    aggressive_prompt: default_aggressive_prompt,  // shorter prompt for old messages
    aggressive_prompt_role: extension_prompt_roles.SYSTEM,  // role for aggressive prompt
    aggressive_prefill: "",   // aggressive prompt prefill
    aggressive_show_prefill: false, // whether to show the prefill for aggressive summaries
    aggressive_prompt_macros: default_summary_macros,  // macros for aggressive prompt
...
class MemoryEditInterface {
    ...
    filter_bar = {
        ...
        "no_summary_bot": {
            "title": "Bot messages without a summary (for bulk summarization)",
            "display": "No Summary (Bot)",
            "check": (msg) => msg && !get_data(msg, 'memory') && !msg.is_user,
            "default": false,
            "count": 0
        },
        "errors": {
...
<div class="flex1 qvink_interface_card">
    ...
    async show() {
        // Reset filters to default to avoid lag on open
        for (let [id, data] of Object.entries(this.filter_bar)) {
    ...
    update_filters(preserve_page = false) {
        ...
        let filter_no_summary_bot = this.filter_bar.no_summary_bot.filtered()
        ...
        for (let i = this.ctx.chat.length - 1; i >= 0; i--) {
            ...
            else if (filter_no_summary_bot && this.filter_bar.no_summary_bot.check(msg)) include = true;
...
<div class="inline-drawer">
    ...
    // Configuration for settings keys (allows reuse for aggressive prompts)
    config = {
        title: 'Summary Prompt',
        prompt_key: 'prompt',
        prompt_role_key: 'prompt_role',
        prefill_key: 'prefill',
        show_prefill_key: 'show_prefill',
        macros_key: 'summary_prompt_macros',
    ...
    async init() {
        ...
        this.$restore.on('click', () => this.$prompt.val(this.config.default_prompt))
    ...
    from_settings() {
        // set the interface from settings using configured keys
        this.$prompt?.val(get_settings(this.config.prompt_key))
        this.$prompt_role?.val(get_settings(this.config.prompt_role_key))
        this.$prefill?.val(get_settings(this.config.prefill_key))
        this.$show_prefill?.prop('checked', get_settings(this.config.show_prefill_key, true))
        this.macros = get_settings(this.config.macros_key, true)
    ...
    save_settings() {
        // save settings in the interface using configured keys
        set_settings(this.config.prompt_key, this.$prompt.val())  // save the prompt
        set_settings(this.config.prompt_role_key, Number(this.$prompt_role.val()))
        set_settings(this.config.prefill_key, this.$prefill.val())
        set_settings(this.config.show_prefill_key, this.$show_prefill.is(':checked'))
        set_settings(this.config.macros_key, this.macros, true)
        update_all_message_visuals()
        debug(get_settings(this.config.macros_key))
    }
    get_prompt_role(name = false) {
        let role = this.is_open() ? Number(this.$prompt_role.val()) : get_settings(this.config.prompt_role_key)
        if (name) {
            switch (role) {
    ...
    async create_summary_prompt(index, prompt = null) {
        ...
        if (prompt === null) {
            prompt = get_settings(this.config.prompt_key)
        }
...
// Summarization
async function summarize_messages(indexes = null, show_progress = true, skip_initial_delay = true, force_aggressive = false) {
    // Summarize the given list of message indexes (or a single index)
    // force_aggressive: if true, all messages will use aggressive prompt regardless of depth
    let ctx = getContext();
    ...
    for (let i of indexes) {
        ...
        await summarize_message(i, force_aggressive);
        n += 1;
    }
...
async function create_aggressive_prompt(index) {
    // Use the aggressive prompt editor interface for full macro support
    return await aggressivePromptEditInterface.create_summary_prompt(index);
}

async function upgrade_to_aggressive() {
    // Find all summaries beyond aggressive depth that were made with standard prompt
    // and re-summarize them with the aggressive prompt
    let context = getContext();
    let aggressive_depth = get_settings('aggressive_depth');
    let indexes_to_upgrade = [];

    for (let i = 0; i < context.chat.length; i++) {
        let message = context.chat[i];
        let depth = context.chat.length - 1 - i;

        // Check if: beyond aggressive depth, has memory, NOT aggressive already
        if (depth > aggressive_depth &&
            get_data(message, 'memory') &&
            !get_data(message, 'is_aggressive')) {
            indexes_to_upgrade.push(i);
        }
    }

    if (indexes_to_upgrade.length === 0) {
        toastr.info('No summaries need upgrading to aggressive mode');
        return;
    }

    let confirm = await getContext().Popup.show.confirm(
        `Upgrade ${indexes_to_upgrade.length} summaries to aggressive mode?`,
        'This will re-summarize old messages with the shorter aggressive prompt.',
        { okButton: 'Upgrade', cancelButton: 'Cancel' }
    );
    if (!confirm) return;

    log(`Upgrading ${indexes_to_upgrade.length} summaries to aggressive mode`);

    // Use summarize_messages with force_aggressive = true for proper setup (preset switching, progress bar, etc.)
    await summarize_messages(indexes_to_upgrade, true, false, true);

    toastr.success(`Upgraded ${indexes_to_upgrade.length} summaries to aggressive mode`);
}

async function summarize_message(index, force_aggressive = false) {
    // Summarize a message given the chat index, replacing any existing memories
    // Should only be used from summarize_messages()
    // force_aggressive: if true, always use aggressive prompt regardless of depth

    let context = getContext();
    let message = context.chat[index]
    let message_hash = getStringHash(message.mes);

    // Determine if we should use aggressive mode based on depth or force parameter
    let depth = context.chat.length - 1 - index;
    let aggressive_depth = get_settings('aggressive_depth');
    let use_aggressive = force_aggressive || (depth > aggressive_depth);

    debug(`Summarizing message ${index} (depth: ${depth}, aggressive_depth: ${aggressive_depth}, use_aggressive: ${use_aggressive})`);
    ...
    let summarizing_text = use_aggressive ? "Summarizing (aggressive)..." : "Summarizing...";
    update_message_visuals(index, false, summarizing_text)
    memoryEditInterface.update_message_visuals(index, null, false, summarizing_text)
    ...
    let prompt;
    if (use_aggressive) {
        // Use the simpler aggressive prompt
        prompt = await create_aggressive_prompt(index);
    } else {
        // Use the full summary prompt
        prompt = await summaryPromptEditInterface.create_summary_prompt(index);
    }
    ...
    if (summary) {
        ...
        set_data(message, 'is_aggressive', use_aggressive)  // track which prompt was used
...
<ul style="text-align: left; font-size: smaller;">
    ...
    // Aggressive Mode
    bind_setting('#aggressive_depth', 'aggressive_depth', 'number');
    bind_function('#edit_aggressive_prompt', () => aggressivePromptEditInterface.show());
    bind_function('#upgrade_to_aggressive', upgrade_to_aggressive);
...
let aggressivePromptEditInterface
jQuery(async function () {
    ...
    aggressivePromptEditInterface = new SummaryPromptEditInterface({
        title: 'Aggressive Prompt',
        prompt_key: 'aggressive_prompt',
        prompt_role_key: 'aggressive_prompt_role',
        prefill_key: 'aggressive_prefill',
        show_prefill_key: 'aggressive_show_prefill',
        macros_key: 'aggressive_prompt_macros',
        default_prompt: default_aggressive_prompt,
    })
